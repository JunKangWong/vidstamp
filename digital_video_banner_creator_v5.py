...
class DigitalVideoBannerGenerator():
    ...
    def load_video(self, input_path='/Users/junkangwong/Documents/github_repo/digital_card/input/sample_vid5.mp4'):
    ...
    def generate_name(self, name):
        name_text=TextClip(name,
                      fontsize=self.name_font_size,
                      color=self.font_color,
                      font=self.font,
                      size=self.input_video_size)
        name_text=name_text.set_duration(self.text_duration).set_position((self.name_pos_x, self.name_pos_y))
    ...
    def generate_table_num(self, no):
        ...
        table_no = TextClip(formatted_table_num,
                            fontsize=self.table_num_font_size,
                            color=self.font_color,
                            font=self.font,
                            size=self.input_video_size)
        table_no = table_no.set_duration(self.text_duration).set_position((self.table_num_pos_x, self.table_num_pos_y))
    ...
    def compose_video(self, name, table_num):
        ...
        final_video = CompositeVideoClip([self.input_video, name, table], use_bgclip=True)
...
if __name__ == "__main__":
    ...
    #TODO: check for duplicated data from excel